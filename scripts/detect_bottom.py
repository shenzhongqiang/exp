...
while k < len(first_order_diff):
    time = first_order_diff[k]["time"]
    diff = first_order_diff[k]["value"]
    data = map(lambda x: x["value"], first_order_diff[k-20:k])
    Y = np.array(data) * 1e4
    avg = np.mean(Y)
    lm.fit(X,Y)
    mse = np.mean((Y - lm.predict(X)) ** 2)
    if lm.coef_[0] > 0.04 and mse < 3 and diff > 0 and abs(avg) < 0.1:
        print "%s\t%0.3f\t%0.3f\t%0.3f\t%0.5f" % (time, lm.coef_[0], mse, avg, diff)
    k+=1
